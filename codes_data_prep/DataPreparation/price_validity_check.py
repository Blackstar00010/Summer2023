...
if mom_anomaly_check:
    close_df = pd.read_csv('../data/preprocessed_wrds/price/close_OG.csv')
    ...
    shits = np.log(ret_df).replace([float('inf'), -float('inf'), 0], float('NaN')).dropna().abs().max().sort_values(
        ascending=False)
    top_ten_shits = ret_df.max().sort_values(ascending=False).index[:1000]
    for i, shit in enumerate(top_ten_shits):
        print(i, shit)

    close_df.loc[:, top_ten_shits[:10]].plot()
    plt.title('Close')
    plt.show()
    (close_df.loc[:, top_ten_shits[:10]] / close_df.loc[:, top_ten_shits[:10]].max()).plot()
    plt.title('Normalised Close')
    plt.show()
    ret_df.loc[:, top_ten_shits[:10]].plot()
    plt.title('Return')
    plt.show()
