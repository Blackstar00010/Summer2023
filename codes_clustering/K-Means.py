...
for k in k_values:
    ...
    clusters = {i: [] for i in range(1, k + 1)}  # Dictionary key-value pair
    for i, cluster in enumerate(cluster_labels):
        clusters[cluster + 1].append(firm_names[i])

    if len(nan_firms) > 0:
        clusters[0] = [firm_names[i] for i in nan_firms]
...
for k, clusters in clusters_k.items():
    ...
    for cluster, firms in clusters.items():
        ...
        print(f'Cluster {cluster}: {firms}')

        # Plot the line graph for firms in each cluster
        if cluster != 0:  # Exclude Cluster 0 from plotting
            for firm in firms:
            ...
            plt.xlabel('Characteristics')
            plt.ylabel('Data Value')
            plt.title(f'Cluster {cluster}, k={k}')
